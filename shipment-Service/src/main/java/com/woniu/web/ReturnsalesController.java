...
//    新增退货单
    @IdempotentToken
    @GlobalTransactional
    @PostMapping("/addReturnSales")
    public ResponseResult<String> addReturnSales(@RequestBody ReturnsalesFo returnsalesFo){
...
//        新增退货单
        ...
        returnsalesService.addReturnsale(returnsales);
...
//            根据订单详情id查询原来的订单详情
           ...
           if (cusOrderDetail.getNum()<0){
               ...
               String createTime = sdf.format(cusorder.getOrdertime());

               String jsonStr="{\"sid\":\""+returnsales.getId()+"\",\"time\":\""+createTime+"\",\"goods\":\"["+OrigcusOrderDetail.getGoodsId()+"-"+OrigcusOrderDetail.getNum()+"]\"}";
...
//      给仓库发消息,创建消息列表，定时器会自动触发
            ...
            String jsonStr="{\"sid\":\""+e.getId()+"\",\"time\":\""+createTime+"\",\"goods\":\"["+e.getGoodsId()+"-"+e.getNum()+"]\"}";
            message.setContent(jsonStr);
            message.setRetryCount(5);   // 消息内容   商品Id  商品数量
            messageMapper.insert(message);
        };
